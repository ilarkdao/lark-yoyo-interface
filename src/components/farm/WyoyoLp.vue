<template>
  <div class="wallet">
    ...
    <!--WYOYO-DAI挖矿-->
    <div class="delegate">
      <p class="titleSelected"> WYOYO-DAI</p>
      <div class="delegatetext round-box">
          {{ $t('message.yours') }}： {{ balanceOfDeposit }} LP<br>
          {{ $t('message.totalLp') }}： {{ balanceOfTotalLp }} LP<br>
  ...
  export default {
    ...
    methods: {
      ...
      async getBalanceAndAllowance(){
				...
				let lpInstance = new this.web4.eth.Contract(abi, this.lpYoyo)
      ...
      async letApprove(){
        ...
        this.isLoading = true
				let abi = this.$store.state.ercAbi
				let lpInstance = new this.web3.eth.Contract(abi, this.lpYoyo)
